...
void quicksort (int size)
{
   ...
   while (unfinished_index>=0 || total<size) { 
     ...
     #pragma omp critical
     {
         ...
         {
            my_index = unfinished_index; //grabbing from stack
            unfinished_index--;
            first = unfinished[my_index].first; //grab here
     ...
     {
	    unfinished_index++;
	    unfinished[unfinished_index].first = q+1; //update here
